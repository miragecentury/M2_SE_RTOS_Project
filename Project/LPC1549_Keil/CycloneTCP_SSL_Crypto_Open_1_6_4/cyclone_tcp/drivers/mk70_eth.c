...
error_t mk70EthInit(NetInterface *interface)
{
   ...
   //Kinetis K70 Ethernet MAC is now ready to send
...
error_t mk70EthSetMacFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      crc = mk70EthCalcCrc(&interface->macFilter[i].addr, sizeof(MacAddr));
