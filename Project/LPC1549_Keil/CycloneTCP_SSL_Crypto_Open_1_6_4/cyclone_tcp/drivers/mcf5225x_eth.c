...
error_t mcf5225xEthInit(NetInterface *interface)
{
   ...
   //MCF5225x Ethernet MAC is now ready to send
...
error_t mcf5225xEthSetMacFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      crc = mcf5225xEthCalcCrc(&interface->macFilter[i].addr, sizeof(MacAddr));
...
void mcf5225xEthWritePhyReg(uint8_t phyAddr, uint8_t regAddr, uint16_t data)
{
   ...
   uint32_t value = MCF_FEC_MMFR_ST(1) | MCF_FEC_MMFR_OP(1) | MCF_FEC_MMFR_TA(2);
   //PHY address
...
uint16_t mcf5225xEthReadPhyReg(uint8_t phyAddr, uint8_t regAddr)
{
   ...
   uint32_t value = MCF_FEC_MMFR_ST(1) | MCF_FEC_MMFR_OP(2) | MCF_FEC_MMFR_TA(2);
   //PHY address
   value |= MCF_FEC_MMFR_PA(phyAddr);
   //Register address
   value |= MCF_FEC_MMFR_RA(regAddr);
