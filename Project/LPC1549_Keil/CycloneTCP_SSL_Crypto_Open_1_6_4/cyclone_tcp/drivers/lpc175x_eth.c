...
error_t lpc175xEthInit(NetInterface *interface)
{
   ...
   //LPC175x Ethernet MAC is now ready to send
...
void ENET_IRQHandler(void)
{
   ...
   if(status & INT_TX_DONE)
   {
      ...
      if(i >= LPC175X_ETH_TX_BUFFER_COUNT) i = 0;
...
error_t lpc175xEthSetMacFilter(NetInterface *interface)
{
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      crc = lpc175xEthCalcCrc(&interface->macFilter[i].addr, sizeof(MacAddr));
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   j = i + 1;
   //Wrap around if necessary
   if(j >= LPC175X_ETH_TX_BUFFER_COUNT) j = 0;
   ...
   uint8_t *buffer, size_t size, size_t *length)
{
   ...
   if(i != LPC_EMAC->RxProduceIndex)
   {
      ...
      //Increment index and wrap around if necessary
      if(++i >= LPC175X_ETH_RX_BUFFER_COUNT) i = 0;
      //Save the resulting value
