...
error_t lpc18xxEthInit(NetInterface *interface)
{
   ...
   //LPC18xx Ethernet MAC is now ready to send
...
error_t lpc18xxEthSetMacFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      crc = lpc18xxEthCalcCrc(&interface->macFilter[i].addr, sizeof(MacAddr));
