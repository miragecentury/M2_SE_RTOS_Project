...
error_t lpc23xxEthInit(NetInterface *interface)
{
   ...
   //LPC23xx Ethernet MAC is now ready to send
...
__irq void lpc23xxEthIrqHandler(void)
{
   ...
   if(status & INT_TX_DONE)
   {
      ...
      if(i >= LPC23XX_ETH_TX_BUFFER_COUNT) i = 0;
...
error_t lpc23xxEthSetMacFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      crc = lpc23xxEthCalcCrc(&interface->macFilter[i].addr, sizeof(MacAddr));
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   j = i + 1;
   //Wrap around if necessary
   if(j >= LPC23XX_ETH_TX_BUFFER_COUNT) j = 0;
   ...
   uint8_t *buffer, size_t size, size_t *length)
{
   ...
   if(i != MAC_RXPRODUCEINDEX)
   {
      ...
      //Increment index and wrap around if necessary
      if(++i >= LPC23XX_ETH_RX_BUFFER_COUNT) i = 0;
      //Save the resulting value
