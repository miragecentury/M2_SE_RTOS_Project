...
bool_t ksz8091EventHandler(NetInterface *interface)
{
   uint16_t value;
...
void ksz8091WritePhyReg(NetInterface *interface, uint8_t address, uint16_t data)
{
   ...
   interface->nicDriver->writePhyReg(KSZ8091_PHY_ADDR, address, data);
}
...
uint16_t ksz8091ReadPhyReg(NetInterface *interface, uint8_t address)
{
   ...
   return interface->nicDriver->readPhyReg(KSZ8091_PHY_ADDR, address);
}
