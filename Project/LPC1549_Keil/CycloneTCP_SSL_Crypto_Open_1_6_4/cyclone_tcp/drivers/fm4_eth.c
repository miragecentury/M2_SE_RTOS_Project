...
error_t fm4EthInit(NetInterface *interface)
{
   ...
   //Spansion FM4 Ethernet MAC is now ready to send
...
error_t fm4EthSetMacFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      crc = fm4EthCalcCrc(&interface->macFilter[i].addr, sizeof(MacAddr));
