...
bool_t ksz8061EventHandler(NetInterface *interface)
{
   uint16_t value;
...
void ksz8061WritePhyReg(NetInterface *interface, uint8_t address, uint16_t data)
{
   ...
   interface->nicDriver->writePhyReg(KSZ8061_PHY_ADDR, address, data);
}
...
uint16_t ksz8061ReadPhyReg(NetInterface *interface, uint8_t address)
{
   ...
   return interface->nicDriver->readPhyReg(KSZ8061_PHY_ADDR, address);
}
