...
error_t pppConnect(NetInterface *interface)
{
   ...
   ipv4GetDnsServer(interface, 0, &context->localConfig.primaryDns);
   context->localConfig.primaryDnsRejected = FALSE;
   ipv4GetDnsServer(interface, 1, &context->localConfig.secondaryDns);
...
void pppProcessFrame(NetInterface *interface, PppFrame *frame, size_t length)
{
   ...
   case PPP_PROTOCOL_CHAP:
      ...
      chapProcessPacket(context, (PppPacket *) frame->data, length);
      break;
#endif
