 ...
 * @param[in] context PPP context
 * @param[in] packet CHAP packet received fom the peer
   ...
   const ChapChallengePacket *challengePacket, size_t length)
{
   ...
   chapSendResponse(context, challengePacket->identifier, md5Context.digest);
   ...
   const ChapResponsePacket *responsePacket, size_t length)
{
   ...
   if(responsePacket->identifier != context->chapFsm.identifier)
   ...
   if(status)
   {
      ...
      chapSendSuccess(context, responsePacket->identifier);
   ...
   else
   {
      ...
      chapSendFailure(context, responsePacket->identifier);
   ...
   const ChapSuccessPacket *successPacket, size_t length)
{
   ...
   //When a packet is received with an invalid Identifier field, the
   //packet is silently discarded without affecting the automaton
   if(successPacket->identifier != context->chapFsm.identifier)
      return ERROR_WRONG_IDENTIFIER;
   ...
   const ChapFailurePacket *failurePacket, size_t length)
{
   ...
   //When a packet is received with an invalid Identifier field, the
   //packet is silently discarded without affecting the automaton
   if(failurePacket->identifier != context->chapFsm.identifier)
      return ERROR_WRONG_IDENTIFIER;
...
error_t chapSendChallenge(PppContext *context)
{
   ...
   challengePacket->identifier = ++context->chapFsm.identifier;
...
error_t chapSendResponse(PppContext *context,
   uint8_t identifier, const uint8_t *value)
{
   error_t error;
   size_t n;
   size_t length;
...
error_t chapSendSuccess(PppContext *context, uint8_t identifier)
{
   error_t error;
   size_t length;
   size_t offset;
   ...
   successPacket->identifier = identifier;
...
error_t chapSendFailure(PppContext *context, uint8_t identifier)
{
   error_t error;
   size_t length;
   size_t offset;
   ...
   failurePacket->identifier = identifier;
...
bool_t chapCheckPassword(PppContext *context, const char_t *password)
{
   ...
   //followed by the challenge value
   md5Init(&md5Context);
   md5Update(&md5Context, &context->chapFsm.identifier, sizeof(uint8_t));
