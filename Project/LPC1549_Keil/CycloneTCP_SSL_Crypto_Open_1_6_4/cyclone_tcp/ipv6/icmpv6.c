   ...
   const NetBuffer *buffer, size_t offset, uint8_t hopLimit)
{
   ...
   case ICMPV6_TYPE_ROUTER_SOL:
      ...
      ipv6ProcessRouterSol(interface, pseudoHeader, buffer, offset, hopLimit);
      break;
#endif
   ...
   const NetBuffer *request, size_t requestOffset)
{
   ...
   if(ipv6IsMulticastAddr(&requestPseudoHeader->destAddr))
   {
      ...
      //sending the Echo Reply message
      error = ipv6SelectSourceAddr(&interface,
         &requestPseudoHeader->destAddr, &replyPseudoHeader.srcAddr);
   ...
   {
      //The destination address of the Echo Request message
