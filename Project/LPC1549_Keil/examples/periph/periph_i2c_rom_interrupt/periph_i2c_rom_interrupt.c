...
#define I2C_ADDR_7BIT_TEMP_SENSOR  (0x90)		/* This is the 7-bit address shifted up 1-bit (orig 0x48) */
#endif
...
#define I2C_XFER_DONE     1
#define I2C_XFER_PENDING  2

/* Interrupt error code (used as semaphore) */
static uint32_t i2c_xfer_error, i2c_xfer_status;
...
static I2C_PARAM_T *I2C_Get_XferData(void)
{
	...
	switch (state) {
		...
		case 1:
			if (i2c_xfer_error)
			I2C_DieError("lcd_init", (ErrorCode_t) i2c_xfer_error);
			state = 2;
		...
		case 4:
			if (i2c_xfer_error)
				I2C_DieError("Temp Read", (ErrorCode_t) i2c_xfer_error);
...
static void I2C_MasterXfer(I2C_PARAM_T *param)
{
	...
	i2c_xfer_status = I2C_XFER_PENDING;
	...
	if (param->num_bytes_send && !param->num_bytes_rec) { /* Transmit only */
		...
		er = LPC_I2CD_API->i2c_master_transmit_intr(i2cHandleMaster, param, &res);
	} else if (!param->num_bytes_send && param->num_bytes_rec) { /* Receive only */
		er = LPC_I2CD_API->i2c_master_receive_intr(i2cHandleMaster, param, &res);
	} else if (param->num_bytes_send && param->num_bytes_rec) {
		er = LPC_I2CD_API->i2c_master_tx_rx_intr(i2cHandleMaster, param, &res);
	} else { /* Empty data(?) */
	...
	while (*((volatile uint32_t *)&i2c_xfer_status) == I2C_XFER_PENDING) {
		__WFI();
	}
}
